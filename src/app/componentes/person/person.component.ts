...
export class PersonComponent implements OnInit {
    ...
    this.typePersonService.list({ not_paginator: true }).subscribe((data) => {
      ...
      if (this.fGRoot && this.id && this.fGRoot.get(this.id)) {
        ...
        if (this.readOnly) {
          ...
          if (typePerson) this.check(typePerson);
        }
      }
    });
  }
  ...
  addMaterial(i: number) {
    ...
    Object.keys(this.materialCurrent).forEach((key: string = "description") => {
      ...
      if (!this.materialCurrent[key]) {
        this.toastr.error(
          "Debe llenar todos los campos para registrar una material, herramienta o equipo"
        );
        error = true;
      }
    });
    ...
    this.fPerson
      .get("materials")
      ?.value.value.push({ ...this.materialCurrent });
    this.materialCurrent = {
      ...{
        description: "",
        mark: "",
        model: "",
  ...
  getPerson(identification_number: string) {
    ...
    this.checkPersonTimeController = setTimeout(() => {
      this.personService
        .getPersonByIdentification(identification_number)
        .subscribe((resp: any) => {
  ...
  private populateMainForm(
    ...
    const fullName =
      ...
      `${personData.name || ""} ${personData.last_name || ""}`.trim();
    ...
    this.fPerson
      .get("movement_type")
      ?.setValue(movement_type === "IN" ? "employee" : "visitor");

    // Cambia aquí: usa company_name si existe, si no company
    this.fPerson
      .get("company_name")
      ?.setValue(personData.company_name ?? personData.company ?? "");
    ...
    this.fPerson
      .get("place_of_reception")
      ?.setValue(personData.default_visit_location);
